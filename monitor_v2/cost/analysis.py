...
- 헤더 라인이나 들여쓰기 나열, 불릿 포인트 형식 모두 금지

=== ■ 이번 달 누계 분석 섹션 작성 ===

반드시 `■ 이번 달 누계 분석` 헤더 한 줄로 시작.

목적: 이번 달 누계의 분포 + 신규로 등장한 항목.
...
■ 이번 달 누계 분석
EC2가 이번 달 누계의 86%($3,432)로 가장 큰 비중을 차지하며 일평균 $429 수준입니다. S3는 이번 달 누계의 8%($310, 일평균 $39 수준), EKS는 이번 달 누계의 4%($145, 일평균 $18 수준), Bedrock은 이번 달 누계의 2%($96, 일평균 $12 수준)입니다. 이번 달 들어 swjeong의 미국 서부 inf2.24xlarge 사용이 처음 등장했고, 어제 단일로 EC2 어제 비용의 48%($53)를 만들었습니다.
...
■ 이번 달 누계 분석
EC2가 이번 달 누계의 53%($1,940)로 가장 큰 비중을 차지하며 일평균 $323 수준이고, S3는 이번 달 누계의 9%($312, 일평균 $52 수준)로 다음입니다. 이번 달 들어 EKS에서 처음 비용($12)이 발생했으며, mhsong이 미국 서부에서 클러스터를 가동한 것으로 보입니다.
...
(주의: 신규 발생이 (없음)이므로 누계 섹션에서 "신규로 등장한 ~" 화제를 꺼내지 말 것. 누계 비중 한두 문장으로 마무리.)
"""
