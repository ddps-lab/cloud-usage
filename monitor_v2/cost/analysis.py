...
다룰 범위:
- 입력의 `=== 이번 달 누계 분석 raw ===` 섹션을 활용해 누계 기준 Top 1~2 서비스의 비중%, 금액, 일평균 서술
  예: "EC2가 이번 달 누계의 86%($3,432)로 가장 큰 비중을 차지하며, 일평균 $429 수준입니다."
- 누계 비중이 작은 서비스는 생략 가능
- 입력의 `=== 이번 달 누계 Top 사용자 raw ===` 섹션을 활용해 **이번 달 비용이 가장 많이 쌓인 사용자 최대 5명**을 **불릿 리스트**로 출력 (가독성을 위해 산문 대신 리스트 사용)
  * 입력에 사용자가 5명 이상이면 상위 5명까지 언급
  * 입력에 사용자가 1~4명만 있으면 그 인원만 언급 (없는 사람을 만들어 채우지 말 것)

  형식 강제 — 다음 구조 정확히 따를 것:
      ▸ 이번 달 Top{N} 사용자                     (N = 실제 출력하는 사용자 수, 예: Top5, Top4, Top3, Top2, Top1)
      • <사용자> — ($금액, X%): <서비스 1>($금액, X%) · <서비스 2>($금액, X%)
      • <사용자> — ($금액, X%): <서비스 1>($금액, X%) · <서비스 2>($금액, X%)
  ...
  * 사용자 이름 뒤 ` — ` (em dash + 공백) 로 분리 후 사용자 누계 `($금액, X%)` 표기
  * 그 뒤 ` : ` 로 분리 후 그 사용자가 쓴 주된 서비스/타입 2~3개를 ` · ` (middle dot) 로 연결
  * 각 서비스/타입 항목도 `($금액, X%)` 둘 다 표기 — `($481, 39%)` 형식 (X% 는 입력 라인 끝의 `(N%)`)
  * 서비스명은 짧게: "EC2", "S3", "Bedrock", "EKS" 식 약어 사용. 인스턴스 타입은 그대로 ("p4d.24xlarge")
  * 데이터 전송 등 IAM agnostic 항목은 짧게 표기 ("미국 서부 데이터 전송")

  ✅ 좋은 예 (형식 정확):
      ▸ 이번 달 Top5 사용자
      • hykim — ($1,239, 30%): EC2 p4d.24xlarge($481, 39%) · EC2 미국 서부 데이터 전송($399, 32%)
      • swjeong — ($916, 22%): EC2 p4d.24xlarge($333, 36%) · EC2 g6e.48xlarge($195, 21%)
      • mhsong — ($914, 22%): EC2 m5.8xlarge($704, 77%) · S3 미국 서부 데이터 전송($155, 17%)
      • yjjung — ($420, 10%): EC2 g5.xlarge($380, 90%) · EBS gp3 볼륨($40, 10%)
      • criu — ($210, 5%): EC2 m5.8xlarge($180, 86%) · EBS 스냅샷($30, 14%)

  ❌ 금지 예:
     - "또한", "한편" 같은 도입어로 산문 형식으로 풀어 쓰기 (반드시 불릿 리스트로)
     - "$481" 만 적고 비중% 누락 — 항상 `($금액, X%)` 둘 다
...
- 입력의 `=== 이번 달 신규 발생 ===` 항목 처리 (라벨 A / B에 따라):
  ...
  → 표현 예: "이번 달 들어 EKS에서 처음 비용($12)이 발생했으며, mhsong이 미국 서부에서 클러스터를 가동한 것으로 보입니다."
  ...
  → 예: "이번 달 들어 swjeong의 미국 서부 inf2.24xlarge 사용이 처음 등장했고, 어제 단일로 EC2 어제 비용의 48%($53)를 만들었습니다."
...
=== 어제 비용 분석 raw ===
EC2  $111.00  ▸ 어제의 88%
    swjeong: 미국 서부(us-west-2) inf2.24xlarge 온디맨드 인스턴스 ×1개 1대 평균 8시간  $53.00 (48%)
    jhpark: 미국 서부(us-west-2) inf2.24xlarge 온디맨드 인스턴스 ×3개 1대 평균 1시간  $22.00 (20%)
    mhsong: 미국 서부(us-west-2) m5.8xlarge 온디맨드 인스턴스 ×5개 1대 평균 30분  $18.00 (16%)
S3  $9.00  ▸ 어제의 7%
    swjeong: 미국 서부(us-west-2) S3 스토리지  $3.00 (33%)
    mhsong: 미국 서부(us-west-2) S3 스토리지  $2.00 (22%)
EKS  $2.00  ▸ 어제의 2%
Bedrock  $1.00  ▸ 어제의 1%

=== 어제 관찰된 신호 ===
- [EC2 페이스] 이번 달 누계 $3,432.00 / 일평균 $429.00 / 어제 $111.00 (일평균의 26%) — 평소보다 낮은 흐름
- [EC2] 미국 서부(us-west-2) inf2.24xlarge 온디맨드 인스턴스 총 4대 ($75.00) — swjeong 1대, jhpark 3대

=== 이번 달 누계 분석 raw ===
EC2  $3,432.00  ▸ 이번 달 누계의 86%  일평균 $429.00 수준
S3  $310.00  ▸ 이번 달 누계의 8%  일평균 $39.00 수준
EKS  $145.00  ▸ 이번 달 누계의 4%  일평균 $18.00 수준
Bedrock  $96.00  ▸ 이번 달 누계의 2%  일평균 $12.00 수준

=== 이번 달 누계 Top 사용자 raw ===
hykim  $1,239.00  ▸ 이번 달 누계의 31%
    EC2 미국 서부(us-west-2) p4d.24xlarge 온디맨드 인스턴스  $481.00 (39%)
    EC2 미국 서부(us-west-2) 데이터 전송  $399.00 (32%)
    S3 미국 서부(us-west-2) 스토리지  $120.00 (10%)
swjeong  $916.00  ▸ 이번 달 누계의 23%
    EC2 미국 서부(us-west-2) p4d.24xlarge 온디맨드 인스턴스  $333.00 (36%)
    EC2 미국 서부(us-west-2) g6e.48xlarge 온디맨드 인스턴스  $195.00 (21%)
mhsong  $914.00  ▸ 이번 달 누계의 23%
    EC2 미국 서부(us-west-2) m5.8xlarge 온디맨드 인스턴스  $704.00 (77%)
    S3 미국 서부(us-west-2) 데이터 전송  $155.00 (17%)
yjjung  $250.00  ▸ 이번 달 누계의 6%
    EC2 미국 서부(us-west-2) g5.xlarge 온디맨드 인스턴스  $230.00 (92%)
    EBS gp3 볼륨  $20.00 (8%)
criu  $180.00  ▸ 이번 달 누계의 5%
    EC2 미국 서부(us-west-2) m5.8xlarge 온디맨드 인스턴스  $150.00 (83%)
    EBS 스냅샷  $30.00 (17%)

=== 이번 달 신규 발생 ===
[기존 서비스 안의 새 조합] EC2  $53.00  swjeong: 미국 서부(us-west-2) inf2.24xlarge 온디맨드 인스턴스 ×1개 1대 평균 8시간

[모범 출력]
어제(2026-05-08) AWS 비용은 $126.14였습니다. 이번 달 8일 동안 $3,983을 사용했으며, 이 추세가 이어지면 월말 약 $13,268이 예상됩니다.

■ 어제 비용 분석
어제 비용은 EC2가 88%($111)로 가장 큰 비중을 차지했습니다. 그 안에서는 swjeong의 미국 서부 inf2.24xlarge 1대($53, 48%)가 1대 평균 8시간 가동되어 가장 큰 비중을 만들었고, jhpark의 inf2.24xlarge 3대($22, 20%)가 1대 평균 1시간 가동되어 다음으로 큰 비중을 차지했으며, mhsong의 m5.8xlarge 5대($18, 16%)도 1대 평균 30분 가동되어 일부를 차지합니다. EC2는 이번 달 일평균 $429 수준으로 발생 중이며 어제 $111은 평소보다 낮은 흐름입니다. S3는 어제 $9가 발생했으며 swjeong의 미국 서부 스토리지($3, 33%)와 mhsong의 스토리지($2, 22%)가 주된 항목입니다. 그 외 EKS $2, Bedrock $1 등 합쳐 약 $3가 함께 발생했습니다.

■ 이번 달 누계 분석
EC2가 이번 달 누계의 86%($3,432)로 가장 큰 비중을 차지하며 일평균 $429 수준입니다. S3는 이번 달 누계의 8%($310, 일평균 $39 수준), EKS는 이번 달 누계의 4%($145, 일평균 $18 수준), Bedrock은 이번 달 누계의 2%($96, 일평균 $12 수준)입니다.

▸ 이번 달 Top5 사용자
• hykim — ($1,239, 31%): EC2 p4d.24xlarge($481, 39%) · EC2 미국 서부 데이터 전송($399, 32%) · S3 미국 서부 스토리지($120, 10%)
• swjeong — ($916, 23%): EC2 p4d.24xlarge($333, 36%) · EC2 g6e.48xlarge($195, 21%)
• mhsong — ($914, 23%): EC2 m5.8xlarge($704, 77%) · S3 미국 서부 데이터 전송($155, 17%)
• yjjung — ($250, 6%): EC2 g5.xlarge($230, 92%) · EBS gp3 볼륨($20, 8%)
• criu — ($180, 5%): EC2 m5.8xlarge($150, 83%) · EBS 스냅샷($30, 17%)

이번 달 들어 swjeong의 미국 서부 inf2.24xlarge 사용이 처음 등장했고, 어제 단일로 EC2 어제 비용의 48%($53)를 만들었습니다.
...
EC2  $181.37  ▸ 어제의 78%
    ...
    mhsong: 미국 서부(us-west-2) m5.8xlarge 온디맨드 인스턴스 ×10개 풀 가동  $43.19 (24%)
    kernel-fusion-benchmark: 미국 서부(us-west-2) inf2.xlarge 온디맨드 인스턴스 ×11개 1대 평균 18시간  $18.81 (10%)
S3  $46.87  ▸ 어제의 20%
    mhsong: 미국 서부(us-west-2) 데이터 전송  $41.49 (89%)

=== 어제 관찰된 신호 ===
- [EC2 페이스] 이번 달 누계 $1,940.00 / 일평균 $323.33 / 어제 $181.37 (일평균의 56%) — 평소보다 낮은 흐름

=== 이번 달 누계 분석 raw ===
EC2  $1,940.00  ▸ 이번 달 누계의 53%  일평균 $323.33 수준
S3  $312.00  ▸ 이번 달 누계의 9%  일평균 $52.00 수준

=== 이번 달 누계 Top 사용자 raw ===
mhsong  $1,800.00  ▸ 이번 달 누계의 49%
    EC2 미국 서부(us-west-2) m5.8xlarge 온디맨드 인스턴스  $1,200.00 (67%)
    S3 미국 서부(us-west-2) 데이터 전송  $310.00 (17%)
kernel-fusion-benchmark  $420.00  ▸ 이번 달 누계의 12%
    EC2 미국 서부(us-west-2) inf2.xlarge 온디맨드 인스턴스  $420.00 (100%)

=== 이번 달 신규 발생 ===
[새 서비스] EKS  $12.40  mhsong: 미국 서부(us-west-2) EKS 클러스터 운영 시간

[모범 출력]
어제(2026-05-06) AWS 비용은 $233.36였습니다. 이번 달 6일 동안 $3,654를 사용했으며, 이 추세가 이어지면 월말 약 $18,200이 예상됩니다.

■ 어제 비용 분석
어제 비용은 EC2가 78%($181)로 가장 큰 비중을 차지했습니다. 그 안에서는 미국 서부의 m5.8xlarge 20대(태그 없음 10대 + mhsong 10대)가 풀 가동되어 합산 $141(78%)로 가장 큰 비중을 만들었고, kernel-fusion-benchmark의 inf2.xlarge 11대($19, 10%)는 1대 평균 18시간 가동되어 다음 비중을 차지합니다. EC2는 이번 달 일평균 $323 수준으로 발생 중이며 어제 $181은 평소보다 낮은 흐름입니다. S3는 어제 $47이 발생했으며 주된 항목은 mhsong의 미국 서부 데이터 전송($41, 89%)입니다.

■ 이번 달 누계 분석
EC2가 이번 달 누계의 53%($1,940)로 가장 큰 비중을 차지하며 일평균 $323 수준이고, S3는 이번 달 누계의 9%($312, 일평균 $52 수준)로 다음입니다.

▸ 이번 달 Top2 사용자
• mhsong — ($1,800, 49%): EC2 m5.8xlarge($1,200, 67%) · S3 미국 서부 데이터 전송($310, 17%)
• kernel-fusion-benchmark — ($420, 12%): EC2 inf2.xlarge($420, 100%)

이번 달 들어 EKS에서 처음 비용($12)이 발생했으며, mhsong이 미국 서부에서 클러스터를 가동한 것으로 보입니다.
...
=== 어제 비용 분석 raw ===
EC2  $180.00  ▸ 어제의 91%
    mhsong: 미국 서부(us-west-2) m5.8xlarge 온디맨드 인스턴스 ×10개 풀 가동  $170.00 (94%)

=== 이번 달 누계 분석 raw ===
EC2  $1,800.00  ▸ 이번 달 누계의 94%  일평균 $180.00 수준
S3  $120.00  ▸ 이번 달 누계의 6%  일평균 $12.00 수준

=== 이번 달 누계 Top 사용자 raw ===
mhsong  $1,820.00  ▸ 이번 달 누계의 95%
    EC2 미국 서부(us-west-2) m5.8xlarge 온디맨드 인스턴스  $1,700.00 (93%)
    S3 미국 서부(us-west-2) 스토리지  $120.00 (7%)
...
■ 이번 달 누계 분석
EC2가 이번 달 누계의 94%($1,800)로 가장 큰 비중을 차지하며 일평균 $180 수준이고, S3는 이번 달 누계의 6%($120, 일평균 $12 수준)로 다음입니다.

▸ 이번 달 Top1 사용자
• mhsong — ($1,820, 95%): EC2 m5.8xlarge($1,700, 93%) · S3 미국 서부 스토리지($120, 7%)

(주의: 신규 발생이 (없음)이므로 누계 섹션에서 "신규로 등장한 ~" 화제를 꺼내지 말 것. 누계 비중 + Top 사용자 불릿으로 마무리.)
(주의: Top1 사용자가 95%로 압도적이므로 1명만 출력. 최대 5명까지 라는 규칙은 "최대"이지 강제 아님.)
"""
...
def _fmt_mtd_breakdown(service_mtd: dict, mtd_total: float, mtd_days_elapsed: int) -> str:
    ...
    Returns:
        예) "EC2  $3,432.00  ▸ 이번 달 누계의 86%  일평균 $429.00 수준"
    """
    ...
    for service, mtd in sorted_svcs:
        ...
        lines.append(
            f"{short}  ${mtd:,.2f}  ▸ 이번 달 누계의 {share:.0f}%  일평균 ${daily_avg:,.2f} 수준"
        )
...
def _fmt_top_users_mtd(rows: list, mtd_total: float) -> str:
    ...
    예:
        swjeong  $245.30  ▸ 이번 달 누계의 32%
            EC2 inf2.24xlarge 시간 ×2개  $120.00 (49%)
            S3 USW2-TimedStorage         $80.00 (33%)
    """
    if not rows:
        return '(없음)'
    blocks = []
    for u in rows:
        ...
        header    = f"{user}  ${user_cost:,.2f}  ▸ 이번 달 누계의 {share:.0f}%"
        lines = []
        for d in u.get('breakdowns', []):
            ...
            lines.append(
                f"    {short_svc} {usage_human}{count_str}  ${d['cost']:,.2f} ({sub_share:.0f}%)"
            )
        if lines:
            blocks.append(header + '\n' + '\n'.join(lines))
        else:
