...
def go_to_glacier(go_to_glacier_bucket_list):
                ...
                for obj in objects.get('Contents', []):
                    ...
                    if object_storage_class != 'GLACIER':
                        try:
                            ...
                            failed_object = [bucket_name, object_key]
                            failed_copy_list.append(failed_object)  # 복사 실패한 파일을 리스트에 추가
                            continue
    ...
    return ordered_result_list, bucket_name_max, failed_copy_list

def generate_message(go_to_glacier_bucket_list, bucket_name_max, failed_copy_list):
    messages = []
    len_bucket_list = len(go_to_glacier_bucket_list)
    len_failed_list = len(failed_copy_list)
    header = "List of S3 Buckets successfully moved to Glacier - [" + str(len_bucket_list) + " buckets]\n"
    ...
    if len_bucket_list == 0:
        ...
        for item in go_to_glacier_bucket_list:
            ...
            if len(message) > 3850:
                ...
                message = ""
        if len_failed_list > 0:
            failed_message = "\n\n※ 아래는 오류로 인해 직접 glacier로 이동해야 하는 객체들입니다."
            for item in failed_copy_list:
                failed_message += f"\n  [{item[0]}] - {item[1]}"
...
def lambda_handler(event, context):
    ...
    result_list, bucket_name_max, failed_copy_list = go_to_glacier(go_to_glacier_bucket_list)
    header, messages = generate_message(result_list, bucket_name_max, failed_copy_list)
