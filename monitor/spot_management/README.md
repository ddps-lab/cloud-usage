...
### 클라우드 트레일 서비스 검색 필터를 위한 DATE 정보 수집
...
- 대규모 실험으로 인해 Timeout과 같은 과부하가 발생하지 않도록 사용량 수집 시간을 4분 30초로 제한한다.
    - 제한된 시간 안에 생성된 메세지는 슬랙으로 보내고, 이후 인스턴스 사용량이 많아 수집을 중단한다는 메세지를 덧붙인다.